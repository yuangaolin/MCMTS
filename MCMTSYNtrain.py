...
from Dataset import MCMTSYNDataset
from torch.nn import MSELoss
import torch
import time
import pandas as pd
from utils.tools import EarlyStopping, set_seed, filter1, CategoricalCrossEntropyLoss
from static.constant import Fold, DrugAE_SaveBase, CellAE_SaveBase, MCMTSYN_SaveBase, MCMTSYN_Result,DrugAE_OutputDim,CellAE_OutputDim
from Models import MCMTSYN, DrugAE, CellLineAE
import numpy as np
...
def fit(model, drugEncoder, cellLineEncoder, train_dataloader, optimizer, mse, cce, test_fold):
    ...
    for i, (x, y) in enumerate(train_dataloader):
        ...
        #out1,  out3 = model(d1_encoder, d2_encoder, c_encoder)
        ...
        #loss = 0.6*(loss1+loss3)+0.4*(loss2+loss4)
...
def validate(model, drugEncoder, cellLineEncoder, validation_dataloader, validation_num, mse, cce):
    ...
    with torch.no_grad():
        ...
        #validation_loss_tol =0.6*(validation_loss1 + validation_loss3) +0.4* (validation_loss2 + validation_loss4)
        validation_loss_tol = validation_loss1 + validation_loss2 + validation_loss3 + validation_loss4
        #validation_loss_tol = validation_loss1  + validation_loss3 
...
def test(model, drugEncoder, cellLineEncoder, test_dataloader, test_num, mse, cce, test_fold):
    ...
    with torch.no_grad():
        ...
        with open(MCMTSYN_Result, 'a') as file:
            file.write("---------- Test ----------\n")
            file.write("test result:\n" + "Synergy:\n" + str(synergy_result) + "\nSensitivity:\n" + str(
                sensitivity_result) + "\n")
            #file.write("test result:\n" + "Synergy:\n" + str(synergy_result)  + "\n")
            
        output_file = 'autodl-tmp/MTL/predictions.txt'

        with open(output_file, 'w') as f:
            f.write("真实值\t预测值\n")  # 可选的表头
            for true, pred in zip(y_true1, y_pred1):
                f.write(f"{true}\t{pred}\n")
        print(f"预测值和真实值已输出到 {output_file}")
...
drugs = pd.read_csv('autodl-tmp/data/drug_features.csv')
print("drugs.shape:", drugs.shape)
cell_lines = pd.read_csv('autodl-tmp/data/cell_line_features.csv')
print("cell_lines.shape:", cell_lines.shape)
summary = pd.read_csv('autodl-tmp/data/oneil_summary_idx.csv')
...
cellLineAE.eval()
print(
    "--------- drugAE(" + str(DrugAE_OutputDim) + ") cellLineAE(" + str(CellAE_OutputDim) + ") ---------")
with open(MCMTSYN_Result, 'a') as file:
    file.write("---------drugAE(" + str(DrugAE_OutputDim) + ") cellLineAE(" + str(
        CellAE_OutputDim) + ") ---------\n")
result_per_fold = []
...
for fold_test in range(0, Fold):
    ...
    hp_i = 0
    result_per_hp = []
    save_path = MCMTSYN_SaveBase + "fold_" + str(fold_test) + ".pth"
    for hyper_parameters in hyper_parameters_candidate:
        ...
        for inner_fold in range(0, Fold):
            ...
            inner_val_dataset = MCMTSYNDataset(drugs, cell_lines, inner_val_summary)
            inner_train_dataset = MCMTSYNDataset(drugs, cell_lines, inner_train_summary)
            ...
            model = MCMTSYN(hyper_parameters['hidden_neurons'], model_input).to(device)
    ...
    train_dataset = MCMTSYNDataset(drugs, cell_lines, train_summary)
    train_num = len(train_dataset)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,
                              num_workers=12, pin_memory=True)
    validation_dataset = MCMTSYNDataset(drugs, cell_lines, validation_summary)
    validation_num = len(validation_dataset)
    validation_loader = DataLoader(validation_dataset, batch_size=batch_size,
                                   shuffle=False)
    test_dataset = MCMTSYNDataset(drugs, cell_lines, test_summary)
    test_num = len(test_dataset)
    test_loader = DataLoader(test_dataset, batch_size=batch_size,
                             shuffle=False)

    model = MCMTSYN(best_hp['hidden_neurons'], model_input).to(device)
    model_es = EarlyStopping(patience=patience)
